...
<template>
    ...
    >
        <template #body>
            <div class="flex gap-4 py-4">
                <UButton
                    :variant="
                        create_transaction.type === 'expense'
                            ? 'subtle'
                            : 'ghost'
                    "
                    color="error"
                ...
                <UButton
                    :variant="
                        create_transaction.type === 'transfer'
                            ? 'subtle'
                            : 'ghost'
                    "
                    color="success"
                        ...
                        dayjs(create_transaction.date).isSame(dayjs(), 'date')
                            ...
                            : dayjs(create_transaction.date)
                              ...
                              : dayjs(create_transaction.date)
                                      ...
                                      .isSame(dayjs(), 'date')
                                ? 'Tomorrow'
                                : dayjs(create_transaction.date).format(
                                      'MMM DD, YYYY',
                                  )
                    "
            ...
            <template v-if="create_transaction.type !== 'transfer'">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <UButton
                        ...
                        size="xs"
                        @click="
                            () => void (dialogs.category = !dialogs.category)
                        "
                    />
                </div>
    ...
    >
        <template v-if="ledger_state.ledger" #body>
    ...
    >
        <template #body>
            <label for="new-category-id" class="-mb-2 text-sm">
                Category Name</label
            >
            <input
    ...
    >
        <template
            #body
            v-if="
                ledger_state.ledger && ledger_state.ledger.accounts.length > 0
            "
        >
            <UButton
